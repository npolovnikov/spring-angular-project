...
public class StudentListDao implements BaseListDao<Student, StudentParams> {
    ...
    private SelectSeekStepN<StudentRecord> getStudentSelect(StudentParams params){
        var condition = STUDENT.DELETED_AT.isNull();
        ...
        if (params.getDateFrom() != null && params.getDateTo() != null){
            condition = condition.and(STUDENT.CREATED_AT.between(params.getDateFrom(), params.getDateTo()));
        }
    ...
    private SortField<?>[] getOrderBy(String orderBy, String orderDir){
        ...
        for (val order: orderArray){
            ...
            if (order.equalsIgnoreCase("createDate")){
                listSortBy.add(asc ? STUDENT.CREATED_AT.asc() : STUDENT.CREATED_AT.desc());
            }
        }
