...
public class TeacherListDao implements BaseListDao<Teacher,TeacherParams> {
    ...
    public Page<Teacher> list(PageParams<TeacherParams> pageParams) {
        ...
        List<Teacher> teachers = listQuery.offset(pageParams.getStart())
                .limit(pageParams.getPage())
                .fetchInto(Teacher.class);

        return new Page<>(teachers, count);
    }
    ...
    private SelectSeekStepN<TeacherRecord> getTeacherSelect(TeacherParams params) {
        ...
        if (params.getBirthDate() != null) {
            condition = condition.and(TEACHER.BIRTH_DATE.eq(params.getBirthDate()));
        }
        ...
        if (params.getCreateDateEnd() != null && params.getCreateDateStart() != null) {
            condition = condition.and(TEACHER.CREATE_DATE.
                    between(params.getCreateDateStart(), params.getCreateDateEnd()));
        }
        if (params.getCreateDateStart() != null && params.getCreateDateEnd() == null) {
            condition = condition.and(TEACHER.CREATE_DATE.greaterThan(params.getCreateDateStart()));
        }
        if (params.getCreateDateStart() == null && params.getCreateDateEnd() != null) {
            condition = condition.and(TEACHER.CREATE_DATE.lessThan(params.getCreateDateEnd()));
        }

        var sort = getOrderBy(params.getOrderBy(), params.getOrderDir());

        return jooq.selectFrom(TEACHER).where(condition).orderBy(sort);
    }
    ...
    private SortField[] getOrderBy(String orderBy, String orderDir) {
        ...
        if (orderBy == null) {
            return asc ? new SortField[]{TEACHER.IDD.asc()} : new SortField[]{TEACHER.IDD.desc()};
        }
